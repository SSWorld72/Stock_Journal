...
export function initDB() {
    ...
    initDBPromise = new Promise((resolve, reject) => {
        // 設置超時保護，避免 IndexedDB 底層死鎖導致整個應用程式卡死
        const timeoutId = setTimeout(() => {
            console.error('[資料庫] 本地資料庫 (IndexedDB) 開啟超時，可能底層發生死鎖（initDB）');
            initDBPromise = null;
            reject(new Error('資料庫開啟超時，請重新啟動瀏覽器'));
        }, 5000);
        ...
        request.onerror = (event) => {
            ...
            initDBPromise = null;
            console.error('[資料庫] 本地資料庫 (IndexedDB) 發生錯誤（initDB）:', event.target.error);
        ...
        request.onblocked = (event) => {
            clearTimeout(timeoutId);
            initDBPromise = null;
            console.warn('[資料庫] 本地資料庫 (IndexedDB) 被其他分頁阻擋（initDB）');
            alert('系統偵測到資料庫被其他分頁鎖定！請關閉其他股海手札的分頁後，重新整理本頁面，以免資料庫升級卡住。');
            reject(new Error('資料庫遭到其他分頁鎖定'));
        };
        ...
        request.onsuccess = (event) => {
            ...
            // 當其他分頁或程序發起刪除/升級請求時，立刻關閉自身的連線以解除鎖定 (Block)
            dbInstance.onversionchange = () => {
                dbInstance.close();
                dbInstance = null;
                console.log('[資料庫] 收到版本變更或刪除請求，已自動關閉連線（onversionchange）');
            };
...
export async function exportChipsJSONL(progressCallback, options = { includeTransactions: true }) {
    ...
    const jsonlStream = new ReadableStream({
        async start(controller) {
            ...
            } catch (e) { console.warn('[資料庫] 匯出設定警告（exportChipsJSONL）:', e); }
            ...
            } catch (e) { console.warn('[資料庫] 匯出休市日警告（exportChipsJSONL）:', e); }
            ...
            if (includeTransactions) {
                ...
                } catch (e) { console.warn('[資料庫] 匯出投資組合警告（exportChipsJSONL）:', e); }
            }
            ...
            if (includeTransactions) {
                ...
                } catch (e) { console.warn('[資料庫] 匯出交易明細警告（exportChipsJSONL）:', e); }
            }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (record._recordType === 'metadata' || record._type === 'metadata') {
                        if (record.type === 'header' && record.totalRecords) {
                            ...
                            try {
                                ...
                                // 檢查並詢問 Private Gas Url (私有雲端備份網址)
                                if (incomingSettings.privateGasUrl && settingsObj.privateGasUrl && incomingSettings.privateGasUrl !== settingsObj.privateGasUrl) {
                                    const msg = `⚠️ 發現備份檔中的「專屬備份 GAS 網址」與本機不同！\n\n[備份] ${incomingSettings.privateGasUrl}\n[本機] ${settingsObj.privateGasUrl}\n\n是否要使用備份的網址「覆蓋」本機網址？\n\n(按「確定」覆蓋，按「取消」保留本機網址)`;
                                    if (!confirm(msg)) {
                                        incomingSettings.privateGasUrl = settingsObj.privateGasUrl;
                                    }
                                } else if (!incomingSettings.privateGasUrl && settingsObj.privateGasUrl) {
                                    incomingSettings.privateGasUrl = settingsObj.privateGasUrl;
                                }

                                // 檢查並詢問 Gas Url (API 報價代理網址)
                                if (incomingSettings.gasUrl && settingsObj.gasUrl && incomingSettings.gasUrl !== settingsObj.gasUrl) {
                                    const msg = `⚠️ 發現備份檔中的「API 報價代理 GAS 網址」與本機不同！\n\n[備份] ${incomingSettings.gasUrl}\n[本機] ${settingsObj.gasUrl}\n\n是否要使用備份的網址「覆蓋」本機網址？\n\n(按「確定」覆蓋，按「取消」保留本機網址)`;
                                    if (!confirm(msg)) {
                                        incomingSettings.gasUrl = settingsObj.gasUrl;
                                    }
                                } else if (!incomingSettings.gasUrl && settingsObj.gasUrl) {
                                    incomingSettings.gasUrl = settingsObj.gasUrl;
                                }
                                ...
                                console.error('[資料庫] 還原 Settings 失敗（processBatch）:', err);
                            }
                    ...
                    console.error('[資料庫] JSON 解析錯誤 (行)（processBatch）:', line, e);
                }
            }
        });
    }
    ...
    try {
        ...
        if (holidaysDataToImport) {
            try {
                ...
                console.error('[資料庫] 還原休市日失敗（importChipsJSONL）:', err);
            }
        }
        ...
        if (newlyAddedTransactions.length > 0) {
            try {
                ...
                console.error('[資料庫] 同步新交易明細失敗（importChipsJSONL）:', err);
            }
            ...
            try {
                ...
                console.error('[資料庫] 儲存設定失敗（importChipsJSONL）:', err);
            }
        }
    } finally {
        reader.releaseLock();
...
export function closeConnection() {
    if (dbInstance) {
        dbInstance.close();
        dbInstance = null;
        console.log('[資料庫] 已關閉本地 IndexedDB 連線（closeConnection）');
    }
}
