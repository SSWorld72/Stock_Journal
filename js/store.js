...
const DEFAULT_SETTINGS = {
    ...
    fugleToken: '', // Fugle API Token
    gasSpreadsheetId: '' // 日誌紀錄用的 Google Spreadsheet ID
};
...
export async function loadSettings() {
    const enforceArrays = () => {
        if (!Array.isArray(settings.portfolios)) settings.portfolios = DEFAULT_SETTINGS.portfolios;
        if (!Array.isArray(settings.transactions)) settings.transactions = DEFAULT_SETTINGS.transactions;
        if (!Array.isArray(settings.customLists)) settings.customLists = DEFAULT_SETTINGS.customLists;
        if (!Array.isArray(settings.brokers)) settings.brokers = DEFAULT_SETTINGS.brokers;
    };
    ...
    try {
        ...
        console.error('[儲存] 載入設定失敗（loadSettings）:', e);
        throw e; // 將錯誤往上拋給 app.js，不要默默吞掉
    }
}
...
export async function saveSettings() {
    try {
        ...
        console.error('[儲存] 儲存設定至本地資料庫 (IndexedDB) 失敗（saveSettings）:', e);
