...
import { initStats, renderStats } from './tabs/stats.js';
        ...
        if (mainContainer) {
            mainContainer.innerHTML = `
                <div class="h-full flex flex-col items-center justify-center text-red-500">
                    ...
                    <p class="font-bold text-lg mb-2">應用程式初始化失敗</p>
                    <p class="text-sm text-slate-500 mb-4">${err.message || err}</p>
                    <button onclick="window.location.reload(true)" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">重新整理網頁</button>
                </div>`;
        } else {
            document.body.innerHTML = `<div style="color:red; padding:20px;"><h1>App Crash</h1><pre>${err.stack}</pre></div>`;
        }
        console.error('[核心] 應用程式初始化失敗（DOMContentLoaded）:', err);
    }
});
