...
function notifyHolidayListeners() {
    holidayListeners.forEach(fn => {
        try { fn(); } catch (e) { console.error('[休市日] 執行監聽器失敗（notifyHolidayListeners）:', e); }
    });
}
...
export function setHolidayReminderSetting(val) {
    try {
        ...
        console.error('[休市日] 儲存休市日設定失敗（setHolidayReminderSetting）:', e);
    }
}
...
export async function persistHolidayState() {
    ...
    try {
        ...
        console.error('[休市日] LocalStorage 儲存休市日快取失敗（persistHolidayState）:', e);
    }
    ...
    try {
        ...
        console.error('[休市日] Settings 儲存休市日快取失敗（persistHolidayState）:', e);
    }
...
export async function initHolidayScheduleAuto() {
    ...
    if (holidayState.holidays.size === 0 || isHolidayReminderDue()) {
        try {
            ...
            console.warn('[休市日] 背景初始化休市日失敗（initHolidaysOnLoad）:', e);
        }
    }
}
