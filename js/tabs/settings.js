...
export function initSettings() {
    ...
    mountDangerZone({
        ...
        dbName: 'StockJournalDB',
        localStorageKey: 'stockJournalSettings',
        preClear: () => {
            if (window.gasBackupInstance && typeof window.gasBackupInstance.stopAutoSync === 'function') {
                window.gasBackupInstance.stopAutoSync();
            }
            closeConnection();
        }
    });    const checkboxes = [
    ...
    // Google Spreadsheet ID (日誌紀錄)
    const gasSpreadsheetIdInput = document.getElementById('setting-gas-spreadsheet-id');
    if (gasSpreadsheetIdInput) {
        gasSpreadsheetIdInput.value = settings.gasSpreadsheetId || '';
        gasSpreadsheetIdInput.addEventListener('input', (e) => {
            settings.gasSpreadsheetId = e.target.value.trim();
            saveSettings();
        });
    }
    ...
    if (document.getElementById('gas-backup-module')) {
        window.gasBackupInstance = new GasBackupModule({
            ...
            titleHtml: `<svg class="w-5 h-5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z"></path></svg> 專屬私有雲端備份`,
            tipBoxType: 'info',
            customDescriptionHtml: `
                <span class="font-medium text-indigo-600">${window.t ? window.t('utils.gasBackup.defaultDescComplete') : '✅ 完整備份：'}</span>${window.t ? window.t('utils.gasBackup.defaultDescNote') : '您的所有交易紀錄、投資組合分類、系統偏好設定。'}<br>
                <span class="text-slate-400 text-[10px]">❌ 刻意排除：市場歷史快取 (EPS/營收/籌碼)、台股行事曆、除錯日誌。<br>
                📄 雲端格式：已排除海量籌碼資料，因此採用標準的 <b>.json</b> 格式 (非串流)，檔案極小且讀取快速。<br>
                (*排除的資料為公開資訊，系統在換新機時會自動重新下載，藉此保持私人備份檔的極致輕量與安全)</span>
            `,
            settingsObj: settings,
            useZip: false, // 不使用 ZIP 壓縮，直接備份 JSON
    ...
    if (btnDownloadChips && startInput && endInput) {
        ...
        btnDownloadChips.addEventListener('click', async () => {
            ...
            try {
                ...
                console.error('[設定] 抓取籌碼資料時發生錯誤（importChipsData）:', e);
    ...
    // Fugle Token
    const fugleTokenInput = document.getElementById('setting-fugle-token');
    if (fugleTokenInput) {
        fugleTokenInput.value = settings.fugleToken || '';
        fugleTokenInput.addEventListener('change', (e) => {
            settings.fugleToken = e.target.value.trim();
            saveSettings();
        });
    }
    ...
    // Mount Shared System Logs Module
    mountSystemLogsUI({ 
        containerId: 'system-logs-module',
        wrapperClass: 'bg-slate-50/50 px-2.5 py-4 sm:p-5 rounded-2xl shadow-sm border border-slate-100'
    });
    ...
    mountDangerZone({
        containerId: 'danger-zone-module',
        dbName: 'StockJournalDB',
        wrapperClass: 'bg-red-50/50 px-2.5 py-4 sm:p-5 rounded-2xl shadow-sm border border-red-100'
    });
...
// renderLogs() has been removed as the UI is now handled by utils/logger.js
...
function initHolidaySettings() {
    ...
    renderHolidayUI = function () {
        ...
        reminderBtns.forEach(btn => {
            ...
            if (val === reminder) {
                btn.className = 'holiday-reminder-btn text-xs px-2.5 py-1 rounded-md border cursor-pointer whitespace-nowrap transition-colors bg-rose-600 text-white border-rose-600';
            } else {
                btn.className = 'holiday-reminder-btn text-xs px-2.5 py-1 rounded-md border cursor-pointer whitespace-nowrap transition-colors bg-white text-slate-600 border-slate-200 hover:border-rose-400';
            }
        });
    };
...
function initFinancialsSettings() {
    ...
    const renderFinancialsUI = async () => {
        try {
            ...
            console.warn('[設定] 渲染財報 UI 失敗（buildStatsTab）:', e);
        }
    };
...
function renderBrokers() {
    ...
    settings.brokers.forEach((broker, idx) => {
        ...
        el.innerHTML = `
            <div class="flex flex-wrap items-center gap-x-2 gap-y-3 mb-4">
                <div class="flex items-center gap-2 order-1">
                    <h4 class="text-lg font-bold ${t.title}">${broker.name}</h4>
                    ${isDefault ? `<span class="${t.badge} text-white text-[10px] px-1.5 py-0.5 rounded">預設</span>` : ''}
                </div>
                
                <div class="flex items-center gap-3 order-2 md:order-3 ml-auto">
                    ${!isDefault ? `<button class="text-xs text-slate-400 hover:${t.accent} transition-colors whitespace-nowrap" onclick="window.settingsTab.setDefaultBroker(${broker.id})">設為預設</button>` : ''}
                ...
                <div class="${t.infoBg} ${t.infoBorder} border px-3 py-1.5 md:px-4 md:py-1.5 rounded-lg flex items-center justify-between md:justify-start gap-4 order-3 md:order-2 w-full md:w-auto">
                    <span class="text-xs md:text-sm text-slate-500">原始資金</span>
                    <span class="font-bold text-sm md:text-base text-slate-800">$${(broker.initialCapital || 0).toLocaleString()}</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 text-sm text-slate-600">
                ...
                <div><span class="${t.stockColor} font-medium">全股</span> 最低：$${broker.minFee || 20}</div>
