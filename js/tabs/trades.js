...
window.getHoldingsAtDate = (code, dateStr, stopId = null, allTrades = null) => {
    let shares = 0;
    // Fix JS Date parsing issue where "YYYY-MM-DD" parses as UTC but "YYYY/MM/DD" parses as Local Time
...
/**
 * 依交易類型計算各欄位的顯示值（typeColor、typeText、displayQty、displayPrice、displayFee、displayTax、displayNhi）
 * @param {object} t - 交易紀錄物件
 * @returns {object} 各欄位顯示值
 */
export function buildTradeDisplayValues(t) {
    ...
    if (t.category === 'trade' || !t.category) {
        ...
        typeColor = isBuy ? 'text-red-600 bg-red-50 border-red-200' : 'text-green-600 bg-green-50 border-green-200';
        ...
        if (t.nhi && t.nhi > 0) {
            displayNhi = `<div class="text-[10px] text-slate-400 mt-0.5">健保 ${t.nhi.toLocaleString()}</div>`;
        }
    } else if (t.category === 'dividend') {
        ...
        typeColor = t.type === 'deposit' ? 'text-red-600 bg-red-50 border-red-200' : 'text-green-600 bg-green-50 border-green-200';
    ...
    return { typeColor, typeText, displayQty, displayPrice, displayFee, displayTax, displayNhi };
}

/**
 * 依交易類型條件渲染手機版交易卡片的資訊欄位 HTML
 * @param {object} t - 交易紀錄物件
 * @param {object} vals - buildTradeDisplayValues(t) 的回傳值
 * @returns {string} HTML 字串
 */
export function buildMobileTradeInfoGrid(t, vals) {
    const { displayQty, displayPrice, displayFee, displayTax, displayNhi } = vals;
    const cat = t.category || 'trade';
    const typ = t.type;

    // 折讓、帳戶存取 → 只顯示總額（已在券商列顯示），info grid 隱藏
    if (cat === 'fee_rebate' || cat === 'account') return '';

    // 買進
    if (cat === 'trade' && typ === 'buy') return `
    <div class="bg-white/40 rounded border border-white/50 p-2.5 text-sm text-slate-700 flex flex-wrap gap-x-4 gap-y-2">
        <div class="flex-[3] min-w-[150px]">
            <div class="flex justify-between items-center mb-1.5">
                <span class="text-slate-400 text-[10px]">數量</span>
                <span class="font-medium">${displayQty}</span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-slate-400 text-[10px]">單價</span>
                <span class="text-slate-500">${displayPrice}</span>
            </div>
        </div>
        <div class="flex-[2] min-w-[130px] border-l border-slate-200/50 pl-4">
            <div class="flex justify-between items-center">
                <span class="text-slate-400 text-[10px]">手續費</span>
    ...
    <div class="bg-white/40 rounded border border-white/50 p-2.5 text-sm text-slate-700 flex flex-wrap gap-x-4 gap-y-2">
        ...
        <div class="flex-[2] min-w-[130px] border-l border-slate-200/50 pl-4">
            ...
            <div class="flex justify-between items-center">
                <span class="text-slate-400 text-[10px]">證交稅</span>
                <span class="text-slate-500">${displayTax}</span>
            </div>
        </div>
    </div>`;

    // 配息 / 配股 / 配息+配股
    if (cat === 'dividend') return `
    <div class="bg-white/40 rounded border border-white/50 p-2.5 text-sm text-slate-700 flex flex-wrap gap-x-4 gap-y-2">
        <div class="flex-[3] min-w-[150px]">
            ${typ !== 'cash' ? `<div class="flex justify-between items-center mb-1.5">
                <span class="text-slate-400 text-[10px]">配股數量</span>
                <span class="font-medium">${displayQty}</span>
            </div>` : ''}
            <div class="flex justify-between items-center">
    ...
    // 減資 / 分割
    if (cat === 'capital_change') return `
    <div class="bg-white/40 rounded border border-white/50 p-2.5 text-sm text-slate-700 flex flex-wrap gap-x-4 gap-y-2">
        <div class="flex-[3] min-w-[150px]">
            <div class="flex justify-between items-center mb-1.5">
                <span class="text-slate-400 text-[10px]">${typ === 'reduction' ? '減資比例' : '分割比例'}</span>
                <span class="font-medium">${displayQty}</span>
            </div>
            ${typ === 'reduction' ? `<div class="flex justify-between items-center">
                <span class="text-slate-400 text-[10px]">退還現金</span>
                <span class="text-slate-500">${displayPrice}</span>
            </div>` : ''}
        </div>
    </div>`;
...
export function initGlobalTradeModal() {
    ...
    const updateFormUI = () => {
        ...
        const dateHint = document.getElementById('trade-date-hint');
        if (dateHint) {
            if (currentTradeType.startsWith('dividend')) {
                dateHint.innerText = '請填寫：除權息日';
            } else if (currentTradeType.startsWith('capital_')) {
                dateHint.innerText = '請填寫：舊股票最後交易日的隔天';
            } else if (currentTradeType === 'buy' || currentTradeType === 'sell') {
                dateHint.innerText = '請填寫：實際交易日 (影響 FIFO 成本與損益)';
            } else {
                dateHint.innerText = '請填寫：實際發生日';
            }
...
export function processCsvContent(csvText) {
    try {
        ...
        for (let i = headerRowIdx + 1; i < lines.length; i++) {
            ...
            if (!typeInfo) {
                console.warn(`[交易] 未知的交易類型（importCSV）: ${typeStr}，略過此列`);
                errorCount++;
                continue;
            }
        ...
        console.error('[交易] CSV 匯入錯誤（importCSV）:', err);
    }
}
...
export function renderTrades() {
    ...
    sortedTrades.forEach((t, index) => {
        ...
        row.className = isChecked ? 'block md:table-row w-full transition-colors group border-b border-slate-100 bg-blue-50' : 'block md:table-row w-full hover:bg-slate-100/50 transition-colors group border-b border-slate-100';
        ...
        row.innerHTML = `
            ...
            <td class="block md:hidden p-1.5">
                <div class="flex items-start justify-between mb-1">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <div class="font-bold text-slate-800 text-base leading-none mb-1">${displayCodeName}</div>
                            <div class="text-[13px] text-slate-500 leading-tight">${t.date}</div>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <button class="text-blue-500 hover:text-blue-700 transition-colors flex items-center gap-1 text-xs font-medium" onclick="if(window.openTradeModal) window.openTradeModal('${t.code || ''}', '${t.id}')">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                            編輯
                        </button>
                        <button class="text-slate-400 hover:text-red-500 transition-colors flex items-center gap-1 text-xs font-medium" onclick="window.tradesTab.deleteTrade('${t.id}')">
                ...
                <div class="flex items-center gap-1.5 mb-1.5">
                    <span class="inline-block px-1.5 py-0.5 ${brokerBadgeStyle} rounded text-[10px] font-medium border whitespace-nowrap leading-none">${brokerName}</span>
                    <span class="inline-block px-1.5 py-0.5 rounded text-[10px] font-medium border ${typeColor} whitespace-nowrap leading-none">${typeText}</span>
                    <div class="ml-auto flex items-center gap-2">
                        <span class="text-[10px] text-slate-400">${(() => {
                            const cat = t.category || 'trade';
                            const typ = t.type;
                            if (cat === 'dividend') {
                                if (typ === 'cash') return '配息';
                                if (typ === 'stock') return '配股';
                                if (typ === 'both') return '配息+配股';
                            }
                            if (cat === 'fee_rebate') return '折讓';
                            if (cat === 'account') return typ === 'deposit' ? '存入' : '取出';
                            if (cat === 'capital_change') return typ === 'reduction' ? '退現' : '總額';
                            return '總額';
                        })()}</span>
                        <span class="font-bold ${amountColor}">${(t.total || 0).toLocaleString()}</span>
                    </div>
                </div>
            ...
            <!-- 電腦版表格 (Desktop Only) -->
            <td class="hidden md:table-cell px-3 py-3 text-center align-middle">
                <input type="checkbox" class="trade-checkbox cursor-pointer w-4 h-4 text-blue-600 bg-slate-100 border-slate-300 rounded focus:ring-blue-500" value="${t.id}" ${selectedTradeIds.has(t.id) ? 'checked' : ''}>
            </td>
            ...
            <td class="hidden md:table-cell px-4 py-2 align-middle">
                <div class="mb-1"><span class="inline-block px-1.5 py-0.5 ${brokerBadgeStyle} rounded text-[10px] font-medium border whitespace-nowrap">${brokerName}</span></div>
                <span class="inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${typeColor} whitespace-nowrap">${typeText}</span>
            </td>
